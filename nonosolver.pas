...
type
  ...
  TNonogramSolver = class(TInterfacedObject,INonogramSolver)
    ...
    function getFirstEmptyCell(entry:TClueOptionEntry):integer;
...
function TNonogramSolver.getFirstEmptyCell(entry: TClueOptionEntry): integer;
var
  index:integer;
begin
  result:=-1;
...
begin
  ...
  while moreOptions do
    begin
    emptyCell:=getFirstEmptyCell(entry);
    moreOptions:=(emptyCell > -1);
    //we have the first space. We can move the clue before this space up by 1
    //remove the empty cell at this position and add it to the start
    entry.moveToStart(emptyCell)
